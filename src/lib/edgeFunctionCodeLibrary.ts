...
export const EDGE_FUNCTION_LIBRARY: EdgeFnLibraryEntry[] = [
  entry("video-proxy",    "Video Proxy",    "⚡ v11 ZERO-WAIT RANGE STREAMING — MP4/MKV range requests stream immediately instead of waiting for a full 16MB proxy window, while playlists/small assets still cache. Fixes HTTP resume, seek, first-frame, and episode switching. Redeploy and paste the URL into EGD Router → video-proxy.", videoProxySource, [], { isNew: true, badgeText: "v11 · NEW", badgeTone: "amber" }),
