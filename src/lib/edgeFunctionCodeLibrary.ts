...
export const EDGE_FUNCTION_LIBRARY: EdgeFnLibraryEntry[] = [
  entry("video-proxy",    "Video Proxy",    "⚡ v9 EDGE CACHE + HIGH-CONCURRENCY — Deno Deploy `caches.open` for aligned MP4 windows, HLS playlists, and HLS segments. One origin fetch feeds every viewer of the same window at the POP → scales to millions of concurrent users with tiny TTFB. Redeploy and paste the URL into EGD Router → video-proxy.", videoProxySource, [], { isNew: true, badgeText: "v9 · NEW", badgeTone: "amber" }),
