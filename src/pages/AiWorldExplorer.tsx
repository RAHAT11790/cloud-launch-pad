...
const normalizeBase = (v: string) => {
  ...
  try {
    ...
    const ends = new Set(["search", "anime", "episode"]);
    while (parts.length && ends.has(parts[parts.length - 1].toLowerCase())) parts.pop();
    url.pathname = `/${parts.join("/")}`.replace(/\/+$/, "");
    return url.toString().replace(/\/+$/, "");
  } catch { return raw.replace(/\/+$/, ""); }
};

export default function AiWorldExplorer() {
  ...
  useEffect(() => {
    document.title = "AI World Explorer";
    getEdgeFunctionUrl("aiworld-api").then((url) => setApiBase(normalizeBase(url)));
    // Auto-load initial list
    (async () => {
      const url = normalizeBase(await getEdgeFunctionUrl("aiworld-api"));
      if (!url) return;
      try {
        setLoading(true);
        const r = await fetch(`${url}/search?q=`);
  ...
  async function doSearch(e?: React.FormEvent) {
    e?.preventDefault();
    if (!apiBase) { setErr("AI World API URL not configured."); return; }
  ...
  async function openAnime(it: SearchItem) {
    ...
    try {
      const r = await fetch(`${apiBase}/anime?id=${encodeURIComponent(it.id)}`);
      const d = await r.json();
  ...
  async function openEpisode(slug: string) {
    ...
    try {
      const r = await fetch(`${apiBase}/episode?slug=${encodeURIComponent(slug)}`);
      const d = await r.json();
      if (d?.error) throw new Error(d.error);
  ...
  return (
    <div className="min-h-screen text-[#e7e9ee] overflow-x-hidden" style={{
      background: "radial-gradient(900px 500px at 10% -10%,#0f2a3a 0%,transparent 60%),radial-gradient(700px 500px at 110% 10%,#2b0f3a 0%,transparent 60%),#080a12",
      fontFamily: "-apple-system,BlinkMacSystemFont,Inter,sans-serif",
    }}>
      <div className="max-w-[1100px] mx-auto px-3 sm:px-4 pt-5 pb-24">
        <header className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl grid place-items-center font-extrabold text-white text-[10px] shrink-0 shadow-[0_8px_28px_rgba(64,196,255,0.35)]"
               style={{ background: "linear-gradient(135deg,#40c4ff,#7c4dff)" }}>AIW</div>
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg m-0 tracking-wide font-semibold truncate">AI World Explorer</h1>
            <div className="text-[11px] sm:text-xs text-[#8b90a0] mt-0.5">Browse → open anime → fetch episode streams</div>
          </div>
        </header>
        ...
        <form onSubmit={doSearch} className="flex gap-2 bg-[#13151d] border border-[#262936] p-2 rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.35)]">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search AI World anime…" autoComplete="off"
          ...
          <button type="submit" className="shrink-0 border-0 text-white px-4 sm:px-5 py-2.5 rounded-lg font-semibold text-sm cursor-pointer"
                  style={{ background: "linear-gradient(135deg,#40c4ff,#7c4dff)" }}>Search</button>
        </form>

        {loading && view === "search" && <div className="text-center text-[#8b90a0] py-8 text-sm">Loading…</div>}
        ...
        {view === "search" && results.length > 0 && (
          <div className="grid gap-2.5 sm:gap-3.5 mt-4 grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(170px,1fr))]">
            {results.map((it) => (<Card key={it.id} it={it} onClick={() => openAnime(it)} />))}
          </div>
        )}

        {view === "anime" && (
          <>
            <button onClick={() => setView("search")} className="bg-transparent border border-[#262936] text-[#e7e9ee] px-3 py-1.5 rounded-lg cursor-pointer text-xs mt-4">← Back</button>
            ...
            {detail && (
              <div className="bg-[#13151d] border border-[#262936] rounded-2xl p-3 sm:p-4 mt-3">
                ...
                {detail.seasons.length === 0 ? (
                  ...
                  <div key={s.name} className="mt-4">
                    ...
                    <div className="grid gap-1.5 grid-cols-[repeat(auto-fill,minmax(60px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(80px,1fr))]">
                      {s.episodes.map((e) => (
                        <button key={e.slug} onClick={() => openEpisode(e.slug)}
                          className={`p-2 text-center border rounded-lg text-xs transition ${episodes[e.slug] ? "bg-[#1a2833] border-[#40c4ff]" : "bg-[#1a1d28] border-[#262936] hover:border-[#40c4ff]"}`}>
                          EP {e.number}
                        </button>
                      ))}
                    </div>
                ...
                {Object.entries(episodes).map(([k, ep]) => (<EpisodePanel key={k} ep={ep} copy={copy} copied={copied} />))}
              </div>
            )}
          </>
        )}
        ...
        <div className="bg-[#0f1119] border border-[#262936] px-3 py-2.5 rounded-xl mt-5 font-mono text-[10px] sm:text-[11px] text-[#a4b1d0] overflow-auto break-all">
          <b className="text-[#40c4ff]">API:</b> {apiBase ? `GET ${apiBase}/search?q=… · /anime?id=… · /episode?slug=animeId:epId` : "AI World API not deployed yet."}
        </div>
      </div>
    </div>
  );
...
function Poster({ src, className }: { src: string; className: string }) {
  ...
  if (failed || !src) return <div className={`${className} bg-gradient-to-br from-[#1a1c25] to-[#2a1d33] grid place-items-center text-[#5a5f70] text-[10px] uppercase tracking-wider`}>No Image</div>;
  return <img src={src} alt="" referrerPolicy="no-referrer" loading="lazy" onError={() => setFailed(true)} className={`${className} object-cover bg-[#1a1c25]`} />;
}

function Card({ it, onClick }: { it: SearchItem; onClick: () => void }) {
  return (
    <div onClick={onClick} className="bg-[#13151d] border border-[#262936] rounded-xl overflow-hidden cursor-pointer hover:-translate-y-1 hover:border-[#40c4ff] transition">
      ...
      <div className="px-2.5 py-2">
        ...
        <div className="text-[10px] sm:text-[11px] text-[#8b90a0] mt-1 flex justify-between items-center gap-1">
          <span className="bg-[rgba(64,196,255,0.18)] text-[#a4dcff] px-1.5 py-px rounded text-[9px] uppercase tracking-wider">{it.type}</span>
          <span>{it.year}</span>
        </div>
      </div>
    </div>
      ...
      {ep.sources.map((s: any, i: number) => (
        <div key={i} className="bg-[#1a1d28] border border-[#262936] rounded-lg p-2.5 sm:p-3 mt-2">
          <div className="flex justify-between items-center text-[11px] text-[#8b90a0] mb-2 gap-2">
            <span className="truncate">Server {i + 1}{s.embed && ` · ${safeHost(s.embed)}`}</span>
            <span className="shrink-0">{s.streams?.length || 0}q · {s.audio?.length || 0}a</span>
          </div>
          {s.master && (
            <div className="bg-gradient-to-r from-[#1d2a33] to-[#1d2433] border border-[#40c4ff]/40 rounded-lg p-2.5 mt-2">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[10px] bg-[#40c4ff]/30 text-[#a4dcff] px-1.5 py-0.5 rounded uppercase tracking-wider font-bold">★ Master</span>
              </div>
              <div className="font-mono text-[10px] text-[#a4b1d0] break-all mb-2">{s.master}</div>
              <div className="flex gap-1.5">
                <button onClick={() => window.open(s.master!, "_blank")} className="flex-1 border-0 text-white px-2 py-1.5 rounded text-[11px] font-semibold cursor-pointer" style={{ background: "#40c4ff" }}>Open</button>
          ...
          {s.embed && !s.master && (
            <div className="mt-2 font-mono text-[10px] text-[#a4b1d0] break-all">{s.embed}</div>
          )}
          {Array.isArray(s.streams) && s.streams.map((q: any) => (
            <Row key={q.url} label={`${q.label || q.resolution || "Video"}`} url={q.url} onCopy={copy} copied={copied} />
          ))}
          {Array.isArray(s.audio) && s.audio.map((a: any) => (
            <Row key={a.uri} label={`🔊 ${a.name || a.language || "Audio"}`} url={a.uri} onCopy={copy} copied={copied} />
          ))}
        </div>
      ))}
    </div>
